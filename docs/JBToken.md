...
```solidity
function burn(uint256, address _account, uint256 _amount) external nonpayable
```
...
```solidity
function mint(uint256, address _account, uint256 _amount) external nonpayable
```
...
*Only the owner of this contract can transfer it.This is necessary to override to adhere to the IJBToken interface.*
